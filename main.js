...
// As soon as a frequency repeats itself (-1, -1), start over from the beginning 
    // Repeat until you get a duplicate frequency
...
const frequency = (freqArray) => {
    ...
    while (finalFrequency[0] === false) {
        for (let i = 0; i < frequencyChanges.length - 1; i++) {
            finalFrequency[1] += freqArray[i] 

            if (finalFreqList.has(finalFrequency[1])) {
                finalFrequency[0] = true
                break
            }
